...
import { FileText, Download, Eye, Calendar, HardDrive, User } from 'lucide-react';
import Breadcrumbs from '../components/Breadcrumbs';
import DocumentViewerModal from '../components/DocumentViewerModal';

export default function VendorDocumentDetails() {
  const { id } = useParams();
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        <Breadcrumbs items={[
          { label: 'Vendors', path: '/documents/vendors' },
          { label: vendorName || 'Vendor Documents' }
        ]} />
      </div>

      <div className="mb-8">
        <h1 className="text-3xl font-bold text-surface-on">{vendorName}</h1>
        <p className="text-surface-on-variant mt-1">All documents uploaded during vendor onboarding and registration.</p>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-lowest border-b border-outline">
                ...
                <th className="py-4 px-6 text-sm font-semibold text-surface-on-variant">Document Type</th>
